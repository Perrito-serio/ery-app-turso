// src/app/api/admin/weekly-stats/route.ts
 ...
 * Protegido con autenticación de administrador por cookies/sesión.
 */
export async function GET(request: NextRequest) {
  try {
    ...
    const completedHabits = habitsWithActivity.reduce((sum, habit) => sum + habit.days_completed, 0);
    const successPercentage = totalHabits > 0 ? Math.round((completedHabits / (totalHabits * 7)) * 100) : 0;
    const totalRelapses = habitsWithActivity.reduce((sum, habit) => sum + (habit.had_relapse ? 1 : 0), 0);
    const bestStreak = Math.max(...habitsWithActivity.map(habit => habit.current_streak), 0);
    ...
    return NextResponse.json(
      { error: 'Error interno del servidor al obtener estadísticas semanales' },
      { status: 500 }
    );
  }
}