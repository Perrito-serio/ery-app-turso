...
// Interfaces
interface AppUser { id: number; nombre: string; email: string; }
interface UserRole { nombre_rol: string; }

const client = new OAuth2Client();

export async function POST(request: NextRequest) {
  // Verificamos que existan las variables que vamos a usar
  ...
  try {
    ...
    let ticket;
    try {
      // Usamos AMBOS Client IDs en la audiencia
      ticket = await client.verifyIdToken({
        idToken,
        audience: [
          process.env.GOOGLE_CLIENT_ID,         // El que usa la web
    ...
    // El resto de la lógica para buscar/crear usuario y generar el token no necesita cambios
    ...
    const jwtToken = jwt.sign(tokenPayload, process.env.NEXTAUTH_SECRET as string, { expiresIn: '30d' });
